...
public:
	...
	size_t FindFirstSubString(size_t begin,const CString str);
...
{
	int i = 0;
	while( (i < this->Length()) && (i < str.Length()) && ( m_arr[i] == str[i]) )
	{
		i++;
	}
	return  (i == this->Length()) && (i == str.Length()) ;
...
size_t CString::FindFirstSubString(size_t begin, const CString str)
{
	CArray<size_t> pos;
	pos = this->FindAllSubStrings(begin, str);
	if( pos.size() == 0)
	{
		return Length();
	}
